...
export const WhyUs = () => {
  const duplicatedPoints = [...whyUsPoints, ...whyUsPoints, ...whyUsPoints, ...whyUsPoints];
  ...
  return (
    <section className="w-full bg-[#f2decc] py-16 md:py-24 px-6 md:px-10 lg:px-[90px] flex flex-col items-center relative z-10 select-none">
      <div className="w-full max-w-[1480px] flex flex-col items-start">
        ...
        {/* Mobile: Infinite Horizontal Marquee Carousel (Visible on < 768px) */}
        <div className="md:hidden block w-full relative overflow-hidden py-4 -mx-6 px-6">
          {/* Edge Fades */}
          ...
          <div className="animate-logo-slide flex gap-6 w-max hover:[animation-play-state:paused] cursor-pointer">
            {duplicatedPoints.map((item, index) => (
              <div 
                key={index}
                className="bg-white border border-black/8 p-8 rounded-[24px] shadow-[0_4px_25px_rgba(0,0,0,0.015)] hover:shadow-[0_16px_35px_rgba(0,0,0,0.045)] transition-all duration-300 flex flex-col justify-between w-[280px] sm:w-[340px] shrink-0"
              >
                <div>
                  <div className="w-[80px] h-[80px] rounded-full border-[2px] border-black flex items-center justify-center mb-6 shrink-0 p-3">
                    <img src={item.icon} alt={item.title} className="w-[50px] h-[50px] object-contain pointer-events-none select-none" />
                  ...
                  <p className="text-[14px] text-black/70 leading-relaxed font-normal select-none">
                    {item.desc}
                  </p>
                </div>
              </div>
